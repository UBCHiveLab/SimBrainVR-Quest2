			...
			for (int i = 0; i < outputs.Count; i++)
			{
				...
				switch (outputs[i].outputMode)
				{
					...
					case OutputMode.ToXML:
						...
						try
						{
							LipSyncClipSetup.SaveFile(settings, outputPath, true, outputs[i].outputClip.transcript, outputs[i].outputClip.length, outputs[i].outputClip.phonemeData, outputs[i].outputClip.emotionData, outputs[i].outputClip.gestureData, outputs[i].outputClip.clip);
						}
						catch (Exception e)
						{
							Debug.LogError(e.StackTrace);
