		...
		public void ProcessData(bool emotionOnly = false)
		{
			if (enabled)
			{
				...
				if (!emotionOnly)
				{
					...
					{
						if (customEmotion == -1)
							tempCurves[index].AddKey(0, blendSystem.GetBlendableValue(tempIndexBlendables[index]));
					...
					for (int index = 0; index < tempEmotionCurves.Count; index++)
					{
						if (customEmotion > -1)
						{
							...
							{
								tempEmotionCurves[index].AddKey(0, blendSystem.GetBlendableValue(tempEmotionIndexBlendables[index]));
							}
						}
						else
						{
					...
					if (useBones && boneCurves != null)
					{
						...
						{
							if (customEmotion == -1)
								tempBoneCurves[index].AddKey(0, tempBones[index].localPosition, tempBones[index].localRotation, tempBones[index].localScale, 0, 0);
							if (!keepEmotionWhenFinished)
						...
						for (int index = 0; index < tempEmotionBoneCurves.Count; index++)
						{
							if (customEmotion > -1)
							{
								...
								{
									tempEmotionBoneCurves[index].AddKey(0, tempEmotionBones[index].localPosition, tempEmotionBones[index].localRotation, tempEmotionBones[index].localScale, 0, 0);
								}
							}
							else
							{
